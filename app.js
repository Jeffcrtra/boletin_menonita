...
function validateFiles(files) {
  ...
  for (const file of files) {
    if (!file.type.startsWith("image/")) {
      return {
        ok: false,
        message: `El archivo "${file.name}" no parece ser una imagen válida.`,
      };
    }
    ...
    if (file.size > MAX_FILE_SIZE_BYTES) {
      return {
        ok: false,
        message: `La imagen "${file.name}" supera el límite de ${MAX_FILE_SIZE_MB} MB.`,
      };
    }
  }
...
function getFormValues() {
  return {
    ...
    ai_aclaracion: document.getElementById("ai_aclaracion").value.trim(),
...
function validateForm(values, files) {
  ...
  if (!values.contenido) return "El contenido no puede ir vacío.";
...
async function saveSubmission(values, imageUrls) {
  const payload = {
    ...
    texto_ai_estado: values.texto_ai_estado,
    imagenes_ai_estado: values.imagenes_ai_estado,
    ai_aclaracion: values.ai_aclaracion || null,
...
form?.addEventListener("submit", async (e) => {
  ...
  setStatus("Subiendo contenido, no cierres esta ventana...", "");
  ...
  try {
    ...
    await saveSubmission(values, imageUrls);

    form.reset();
    clearPreviews();
    setStatus("Contenido enviado correctamente.", "success");
  } catch (error) {
    console.error(error);
